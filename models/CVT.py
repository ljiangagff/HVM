...
class HV_attention(nn.Module):
    ...
    def __init__(self, channels, pool_stride, heads, alpha=0.75, window=4, full_attn=False):
    ...
    def forward(self, x):
        ...
        if self.channels2:
            ...
            va = self.vfc_o(va)
            # result [n, b, c]
            result = torch.cat([ha, va], axis=2)
        else:
            result = self.horizontal_attention(x)
        # [b, h, w, c] tp [b, c, h, w]
        result = result.view(h, w, b, c).permute(2, 3, 0, 1)
        result = result + x
