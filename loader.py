...
class Loader():
    def __init__(self, image_dir=IMAGE_DIR, label_dir=LABEL_DIR, data_dir=DATA_DIR, categorys=['train', 'validate', 'test']) -> None:
    ...
    def load_datas(self, categories: list, device, batch):
            ...
            for fn in os.listdir(local_dir):
                ...
                if filetype == IMAGE_TYPE:
                    ...
                    if 'image' not in datasets[width]:
                        ...
                        datasets[width]['image'] = np.concatenate((datasets[width]['image'], image_np_array), axis=0)
        ...
        print(f'Loaded dataset for {categories}. Loading time {time.time()-start} seconds')
        return t_datasets
    ...
    def load_data(self, category: str, device, batch):
        ...
        print(f'Loaded dataset for {category}. Loading time {time.time()-start} seconds')
        return t_datasets
