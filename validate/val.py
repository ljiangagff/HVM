...
from .search_algorithms import beam_search, greedy_search, greedy_search_batch
from .standardization import clean_seq, filter_blank
...
class DecodingVal():
    ...
    def greedy_validate(self, val_model, is_test=False):
        bleu_total = 0.0
        ...
        for width, val in self.dataset.items():
            for inputs, labels, indices in iter(val):
                inputs, _ = val_model.image_encoder(inputs)
                predictions = greedy_search_batch(val_model, inputs)
                predictions.detach().cpu()
                for i in range(len(indices)):
                    ...
                    decoded_seq = get_decoded(prediction)
                    ...
                    # Bleu without blank
                    decoded_seq = filter_blank(decoded_seq)
                    truth_seq = filter_blank(truth_seq)
                    bleu_no_blank = compute_bleu(truth_seq, decoded_seq)
                    ...
                    bleu_num += 1
                # print(f'Validate for a new test batch completed, current items {bleu_num}, current bleu {bleu_total/bleu_num}')
